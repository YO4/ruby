  ...
  def test_sysseek_after_rewind_with_pending_char
    make_tempfile {|t|
      open(t.path, "rt") {|f|
        ...
        f.ungetc('a')
        f.rewind
        assert_nothing_raised(IOError) {f.sysseek(0)}
      }
    }
  end
      ...
      ["rb", "r+b"].each {|mode|
        open("src", mode) {|f|
          ...
          IO.pipe {|r, w|
            ...
            w.close
            f.reopen(r)
            assert_equal("ABC", f.read, "reopen a #{mode} IO")
          }
        }
      }
    }
  ...
  def test_reopen_io_with_pending_byte_then_write
    mkcdtmpdir {
      ...
      ["rb", "r+b"].each_with_index {|mode, i|
        ...
        assert_equal("XXXXXXXXY", File.binread(dst), "reopen a #{mode} IO")
      }
    }
  end
