...
#else /* _WIN32 */
/* Windows ConPTY support: the child is attached via PSEUDOCONSOLE (see
 * rb_w32_spawnspec_enable_pty); master talks to conpty over two pipes
 * whose master-side ends become the PTY.spawn IOs. */
...
/* Pseudoconsole handles by child pid, closed (ending conhost) on reap. */
...
pty_getpty(int argc, VALUE *argv, VALUE self)
{
    ...
    struct pty_info info;
    rb_pid_t pid = -1;
    /* Two pipes: conpty-side ends go to CreatePseudoConsole, master-side
     * ends stay here as the PTY IOs. */
    ...
    /* Non-inheritable pipes (EchoCon sample): conpty dups its ends into
     * conhost, so inheritable handles would break the attach. */
    ...
    /* Conpty dup'ed its ends into conhost; close our copies so the child
     * attaches to the pseudoconsole instead of inheriting our console. */
    CloseHandle(hInputRead);
    CloseHandle(hOutputWrite);

    master_in_fd = _open_osfhandle((intptr_t)hInputWrite, O_WRONLY | O_NOINHERIT);
    if (master_in_fd < 0) {
    ...
    master_out_fd = _open_osfhandle((intptr_t)hOutputRead, O_RDONLY | O_NOINHERIT);
    if (master_out_fd < 0) {
        int e = errno;
        close(master_in_fd);
        pClosePseudoConsole(hPC);
    ...
    if (!actions) {
        VALUE chdir_dir = eargp->chdir_dir;
        int e = errno;
