...
#if defined(_WIN32)
/*
 * Translate the rb_execarg redirects into builder API (fd details stay in
 * win32.c).  Release the result with rb_w32_spawnspec_destroy.
 */
...
rb_w32_spawnspec_build(const struct rb_execarg *eargp)
{
    ...
    /* Hand the spawn env options to the child via lpEnvironment instead of
     * mutating ours.  With no env options, pass NULL to inherit unchanged. */
    {
        ...
        if (unsetenv_others || envopts != Qfalse) {
            ...
            /* Flatten envtbl into a "KEY=VALUE\0" buffer plus a pointer array. */
    ...
    /* Pass :chdir to the child via lpCurrentDirectory (NULL inherits).  A
     * bad directory fails the build so the spawn raises. */
    if (eargp->chdir_given) {
    ...
    /* Pass :new_pgroup through for CREATE_NEW_PROCESS_GROUP in CreateChild. */
...
save_redirect_fd(int fd, struct rb_execarg *sargp, char *errmsg, size_t errmsg_buflen)
{
    if (sargp) {
        ...
        if (save_fd == -1) {
            if (errno == EBADF) {
                /* Unopened fd: queue fd_close so the child drops it (on
                 * Windows the parent is never touched to begin with). */
...
#if !defined(HAVE_WORKING_FORK) && !defined(_WIN32)
        /* Windows passes the directory via lpCurrentDirectory instead, so the
         * parent must not chdir here (it would also re-base relative paths). */
