...
struct rb_w32_inherit_state {
    ...
    int use_std_handles;        /* set STARTF_USESTDHANDLES: a std handle is
                                 * handed to the child. */
...
{
    /* Read the CRT _osfile entry, guarded: _osfile() may read garbage for an
     * unallocated fd (returns 0 then). */
                 ...
                 WORD *cbReserved2_out)
{
    ...
    *cbReserved2_out = 0;
    ...
    /* cbReserved2 is a WORD: fail fast if the table would overflow it. */
                      ...
                      struct rb_w32_inherit_state *st)
{
    ...
    int std_targeted[3] = {
        spawnspec_targets_fd(actions, 0),
    ...
    st->hStd[2] = hError ? hError
        : (st->reserved2 && st->inherit_count > 2 &&
           ...
           (st->p_flags[2] & FOPEN))
          ? (HANDLE)st->p_handle[2]
          : std_targeted[2] ? NULL : GetStdHandle(STD_ERROR_HANDLE);

    /* Remember handed-over std handles (a NULL hStd of an explicitly
     * closed fd still counts). */
